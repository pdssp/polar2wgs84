...
from polar2wgs84.monitoring import UtilsMonitoring


# Mock logger to prevent actual logging during tests
@pytest.fixture(autouse=True)
def mock_logger(mocker):
    mock_logger = mocker.patch("polar2wgs84.monitoring.logger", new_callable=MagicMock)
    yield mock_logger
...
def test_io_display_logs_inputs_and_outputs(mock_logger):
    ...
    mock_logger.log.assert_any_call(
        "DEBUG", "Input in the function 'test_func' with args=(3, 4), kwargs={}"
    )
    mock_logger.log.assert_any_call(
        "DEBUG", "Output in the function 'test_func' with result=7"
    )


@patch("polar2wgs84.monitoring.logger")
def test_io_display_logs_only_inputs(mock_logger):
    ...
    mock_logger.log.assert_any_call(
        "INFO", "Input in the function 'test_func' with args=(3, 4), kwargs={}"
    )
    ...
    assert not any(
        call[0] == ("INFO", output_log_message)
...
def test_io_display_logs_only_outputs(mock_logger):
    ...
    mock_logger.log.assert_any_call(
        "INFO", "Output in the function 'test_func' with result=7"
    )
...
def test_time_spend_logs_function_duration(mock_logger):
    ...
    mock_logger.log.assert_any_call("INFO", ANY)  # Use ANY to match any string
